...
사용합니다. 지정 채널의 전체 업로드를 가져오지 않고 전역 검색 결과를 기존 이슈·인물
관련도 규칙으로 다시 선별합니다. Shorts는 포함하고, 라이브·예약·라이브 다시보기는
제외합니다. 최초 실행은 최근 90일을 조회하고 이후에는 24시간 겹침 구간을 두어
재수집합니다. 운영 워크플로는 매일 09:17, 21:17(KST)에 실행됩니다. 검색 API 쿼터를
통제하기 위해 기본값은 검색어별 최대 2페이지입니다.
