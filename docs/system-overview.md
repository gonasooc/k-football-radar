...
Next.js 화면은 런타임에서 외부 API를 직접 호출하지 않는다. 대신 빌드할 때 저장소의 JSON 파일을 import해서 읽는다.
...
lib/data.ts
   ↓
app/page.tsx, app/feed/page.tsx, app/issues/page.tsx ...
```
...
오늘의 대시보드다. 최근 24시간 신규 자료, 공식자료 수, 뉴스 수, 많이 언급된 이슈와 인물, 최신 수집 항목을 보여준다.

### `/feed`

전체 수집 피드다. 사용자는 유형, 이슈, 인물, 키워드로 항목을 필터링할 수 있다. 필터 UI는 클라이언트 컴포넌트인 `components/FeedClient.tsx`가 담당한다.
