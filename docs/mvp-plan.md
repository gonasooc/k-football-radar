...
## 2026-07-17 YouTube 확장 확정안

초기 MVP 위에 다음 구성을 추가한다.

- 상단 메뉴를 `홈 / 뉴스 / 유튜브 / 트래킹 / 출처`로 운영한다.
- 기존 홈의 뉴스·공식자료 검색 피드는 `/news`로 그대로 옮긴다.
- 새 홈은 검색 없이 최신 뉴스·공식자료 6개와 최신 유튜브 영상 6개를 별도 흐름으로
  보여주고 각 메뉴의 더보기 링크를 제공한다.
- `/youtube`는 뉴스와 검색·범위·정렬·이슈·인물 필터를 공유하되 유형 필터는 생략하고,
  카드에 16:9 썸네일, 채널, 발행일, 재생 시간을 표시한다.
- YouTube Data API의 전역 키워드 검색을 사용한다. 지정 채널 전체 업로드 수집은 하지
  않으며, 추후 선호 채널 가중치는 별도 확장으로 남긴다.
- Shorts는 포함하고 라이브·예약·라이브 다시보기는 제외한다.
- 최초 실행은 최근 90일을 검색하고 이후 하루 두 번 수집한다. 기본 검색어별 최대
  2페이지로 쿼터를 제한하며, 더 깊은 백필은 날짜 구간을 나눠 수동 실행한다.
- 유튜브 항목은 기존 이슈·인물 분류와 노이즈 필터를 통과해야 하며 트래킹 화면,
  출처 화면, R2 snapshot과 데이터 검증에 모두 포함한다.
- 영상 파일과 자막은 저장하지 않고 YouTube 원본 링크와 API 메타데이터만 저장한다.

## 2026-07-18 YouTube 큐레이션 보완안

위 초기안의 채널·Shorts 정책은 다음 결정으로 대체한다.

- 유튜브 탭은 긴 분석·인터뷰를 중심에 두되 방송사의 짧은 일반 영상도 배제하지 않는다.
- 채널 ID만 담은 `preferred / blocked` 배열과 목록 외 `unlisted` 기본값을 사용한다.
  채널명·검토 메타데이터는 정책 파일에 저장하지 않으며, 자동화는 후보 보고서만 만들고
  채널 상태를 바꾸지 않는다.
- `preferred` 채널의 최근 업로드를 독립적으로 수집하고, 전역 검색으로 찾은 나머지 영상과
  영상 ID 기준으로 합친다. 모든 영상은 주제 관련도 검사를 통과해야 한다.
- 채널 신뢰도와 콘텐츠 관련도를 분리한다. `preferred + primary`만 primary이며,
  `unlisted`는 관련성이 높아도 최대 secondary다. `blocked`는 제외한다.
- 최소 영상 길이는 두지 않는다. 명시적 메타데이터 또는 영상 ID가 일치하는 Shorts
  canonical·Open Graph 주소로 확정한 Shorts만 제외하며, 불명확하면 유지한다.
- 유튜브 화면은 검색창 대신 채널 검색 기능을 추가하지 않고 기존 검색과 함께
  `선별 채널 / 전체 채널` 범위 필터를 제공한다. 홈 영상은 선별 채널만 보여준다.
- 기존 유튜브 데이터도 같은 기준으로 재분류하며, 선별 해제는 secondary 강등,
  blocked 지정은 제거로 반영한다.
...
- 이슈 태그: `issues.json.keywords`와 이슈별 조합 규칙이 같은 제목 또는 설명 문장 안에서 충족되고 제외 문맥이 없으면 부여
- 인물 태그: `people.json.keywords`가 제목/설명에 포함되면 부여
- 공식자료 여부: 공식자료 수집기 또는 출처 도메인 기반으로 판단
- relevanceScore: 제목과 설명의 근거 위치를 구분한 규칙 기반 점수
...
제목의 인물 매칭: 인물당 +8
설명의 인물 매칭: 인물당 +3
해명/사퇴/감사/선거인단 등 고관심 키워드: 제목 +5, 설명 +2
```
...
7. YouTube 선별 채널 후보 보고서와 검색 품질 모니터링 고도화
8. 검색 고도화
9. 인물별 공식 발언/해명 분리
10. 커스텀 도메인 연결
