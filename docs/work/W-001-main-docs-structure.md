...
- 사람이 판단할 사항: 문서 구조 결정 3건(`docs/remaining-work.md`의 위치, 기본 문서와 기존 상세 문서의 담당 경계, `.gitignore` 병합 범위). `docs/plan.md`의 결정 사항은 2026-09-21에 정리됐고, 보존 상한만 [docs/work/W-002-main-item-retention-review.md](W-002-main-item-retention-review.md)로 분리했다. `docs/design.md`의 불일치 4건은 문서 현행화로 해소했다.
...
- [ ] [docs/remaining-work.md](../remaining-work.md)의 위치 결정. 저장소 밖 외부 작업과 정기 점검을 담고 있어 docs-starter의 ‘별도 TODO 파일은 만들지 않는다’ 규칙과 겹친다. 운영 점검 문서로 남길지, 작업 문서로 옮길지 정한다
- [ ] 기본 문서와 기존 상세 문서의 담당 경계 확정: `docs/architecture.md` ↔ [docs/system-overview.md](../system-overview.md), `docs/specs.md` ↔ 루트 `README.md`의 명령·환경변수 설명, `docs/plan.md` ↔ [docs/mvp-plan.md](../mvp-plan.md). 같은 사실이 바뀔 때 어느 문서를 고칠지 한 곳에 적는다
- [ ] `.gitignore`에 템플릿의 `Thumbs.db`를 넣을지 결정 (현재 미반영, macOS 전용 저장소라 생략 가능)
- [ ] 루트의 빈 `sessions/decisions/` 디렉터리 정리 여부 결정. Git에 추적되지 않는 로컬 디렉터리이며 새 `docs/sessions/`와 이름이 겹친다
- [ ] 사용자가 요청하면 문서 변경을 커밋

재개에 필요한 코드 상태: `main` 브랜치, 문서와 `.gitignore`·`README.md`만 미커밋 변경. 코드와 데이터는 건드리지 않았다.
