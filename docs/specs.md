...
`check:readiness`는 GitHub 시크릿·변수와 최신 CI·수집 워크플로 결과를 확인한다. 확인 대상 목록은 [docs/system-overview.md](system-overview.md)의 ‘준비 상태 확인’에 있다.
...
마지막 전체 확인은 2026-07-28이다. 당시 health가 200과 `data.source: "r2"`, `data.stale: false`를 반환했고, 두 수집 워크플로가 예약 실행으로 계속 성공하며 같은 실행에서 R2 snapshot이 발행되는 것, 앱 이미지를 교체하지 않아도 health의 수집 시각이 갱신되는 것을 확인했다.

### 유튜브 과거 구간 추가 수집

필요할 때만 하는 선택 작업이다. 기본 수집은 검색어별 2페이지로 쿼터를 제한하므로, 100건을 넘는 검색어의 과거 구간을 더 채우려면 기간을 나눠 수동 실행한다.
